...
export function useDocumentUpload() {
    ...
    const [documents, setDocuments] = useState<Map<string, DocumentStatus>>(new Map());
    const [error, setError] = useState<string | null>(null);
...
export function useStatutes() {
    ...
    const fetchStatutes = useCallback(async (actCode?: string, domain?: string) => {
        setLoading(true);
        setError(null);
        ...
        try {
            ...
            const errorMessage = err instanceof Error ? err.message : 'Failed to fetch statutes';
            setError(errorMessage);
        } finally {
            setLoading(false);
        }
    ...
    const searchStatutes = useCallback(async (query: string, actCodes?: string[]) => {
        setLoading(true);
        setError(null);
...
export function useIPCBNSComparison() {
    ...
    const fetchComparisons = useCallback(async (ipcSection?: string, bnsSection?: string) => {
        setLoading(true);
        setError(null);

        try {
            const { comparisons: data } = await api.getIPCBNSComparisons(ipcSection, bnsSection);
            setComparisons(data);
        } catch (err) {
            const errorMessage = err instanceof Error ? err.message : 'Failed to fetch comparisons';
            setError(errorMessage);
        } finally {
            setLoading(false);
        }
